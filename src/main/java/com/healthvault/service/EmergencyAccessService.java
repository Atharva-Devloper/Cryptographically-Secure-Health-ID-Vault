...
public class EmergencyAccessService {
    ...
    public String generateEmergencyAccess(int userId, String reason) throws Exception {
        ...
        if (hasActiveEmergencyAccess(userId)) {
            throw new Exception("User already has active emergency access");
        }
        
        String emergencyCode = HealthIdGenerator.generateEmergencyCode();
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsInserted > 0) {
                AuditLogger.logUserAction(userId, "EMERGENCY_ACCESS_GENERATED", "USER", userId,
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    AuditLogger.logEmergencyAccess(info.getUserId(), emergencyCode, accessedBy);
                    
                    return info;
                }
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Emergency access validation failed: " + e.getMessage());
        }
    ...
    public List<EmergencyAccessInfo> getEmergencyAccessHistory(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Emergency access history failed: " + e.getMessage());
        }
    ...
    public boolean revokeEmergencyAccess(int userId, String emergencyCode) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logUserAction(userId, "EMERGENCY_ACCESS_REVOKED", "USER", userId,
                                        String.format("Emergency access revoked: %s", emergencyCode));
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Emergency access revocation failed: " + e.getMessage());
        }
    ...
    public boolean extendEmergencyAccess(int userId, String emergencyCode, int additionalHours) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logUserAction(userId, "EMERGENCY_ACCESS_EXTENDED", "USER", userId,
                                        String.format("Emergency access extended: %s, Hours: %d", emergencyCode, additionalHours));
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Emergency access extension failed: " + e.getMessage());
        }
    ...
    public int cleanupExpiredEmergencyAccess() {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logSystemMaintenance("CLEANUP_EMERGENCY_ACCESS", 
                                              String.format("Cleaned up %d expired emergency access codes", rowsUpdated));
            }
            
            return rowsUpdated;
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Emergency access cleanup failed: " + e.getMessage());
        }
    ...
    public Optional<EmergencyAccessInfo> getActiveEmergencyAccess(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Get active emergency access failed: " + e.getMessage());
        }
    ...
    public String generateEmergencyPassword(int userId, String staffName, String hospitalName) {
        ...
        AuditLogger.logEmergencyAccess(userId, "TEMP_PASSWORD_" + tempPassword, 
    ...
    private boolean hasActiveEmergencyAccess(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Active emergency access check failed: " + e.getMessage());
        }
    ...
    private void markEmergencyAccessAsUsed(int emergencyAccessId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Mark emergency access as used failed: " + e.getMessage());
        }
    }
