...
public class AccessControlService {
    ...
    public boolean hasPermission(int userId, Permission permission, ResourceType resourceType, int resourceId) {
        ...
        Optional<User> userOpt = new UserService().getUserById(userId);
    ...
    public boolean grantPermission(int granterId, int userId, Permission permission, 
        ...
        if (!hasPermission(granterId, Permission.GRANT_PERMISSION, resourceType, resourceId)) {
            AuditLogger.logSecurityEvent("UNAUTHORIZED_PERMISSION_GRANT", 
                                      String.format("User %d attempted to grant permission to user %d", granterId, userId), 
                                      null);
            return false;
        }
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsInserted > 0) {
                AuditLogger.logUserAction(granterId, "PERMISSION_GRANTED", resourceType.name(), resourceId,
                                        String.format("Granted %s permission to user %d", permission.name(), userId));
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Permission grant failed: " + e.getMessage());
        }
    ...
    public boolean revokePermission(int revokerId, int userId, Permission permission, 
        ...
        if (!hasPermission(revokerId, Permission.REVOKE_PERMISSION, resourceType, resourceId)) {
            AuditLogger.logSecurityEvent("UNAUTHORIZED_PERMISSION_REVOKE", 
                                      String.format("User %d attempted to revoke permission from user %d", revokerId, userId), 
                                      null);
            return false;
        }
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logUserAction(revokerId, "PERMISSION_REVOKED", resourceType.name(), resourceId,
                                        String.format("Revoked %s permission from user %d", permission.name(), userId));
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Permission revoke failed: " + e.getMessage());
        }
    ...
    public List<UserPermission> getUserPermissions(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Get user permissions failed: " + e.getMessage());
        }
    ...
    public List<UserPermission> getResourcePermissions(ResourceType resourceType, int resourceId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Get resource permissions failed: " + e.getMessage());
        }
    ...
    public int cleanupExpiredPermissions() {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logSystemMaintenance("CLEANUP_EXPIRED_PERMISSIONS", 
                                              String.format("Cleaned up %d expired permissions", rowsUpdated));
            }
            
            return rowsUpdated;
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Cleanup expired permissions failed: " + e.getMessage());
        }
    ...
    private boolean hasRolePermission(User.UserType userType, Permission permission, ResourceType resourceType) {
        switch (userType) {
            case DOCTOR:
    ...
    private boolean hasFilePermission(int userId, Permission permission, int fileId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "File permission check failed: " + e.getMessage());
        }
        
        // Check if file is shared with user
        return new SharingService().hasFileAccess(userId, fileId);
    }
    ...
    private boolean hasSystemPermission(User.UserType userType, Permission permission) {
    ...
    private boolean permissionExists(int userId, Permission permission, ResourceType resourceType, int resourceId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Permission existence check failed: " + e.getMessage());
        }
    ...
    private boolean updateExistingPermission(int userId, Permission permission, ResourceType resourceType, 
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Update existing permission failed: " + e.getMessage());
        }
