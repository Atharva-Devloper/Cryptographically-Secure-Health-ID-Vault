...
public class SharingService {
    ...
    public boolean shareFileWithUser(int ownerId, int fileId, int sharedWithUserId, 
        ...
        if (!verifyFileOwnership(ownerId, fileId)) {
            AuditLogger.logSecurityEvent("UNAUTHORIZED_SHARING_ATTEMPT", 
                                      String.format("User %d attempted to share file %d", ownerId, fileId), 
                                      null);
            return false;
        }
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsInserted > 0) {
                AuditLogger.logFileSharing(ownerId, fileId, "User ID: " + sharedWithUserId, "SHARED");
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "File sharing failed: " + e.getMessage());
        }
    ...
    public String createTemporaryShareLink(int ownerId, int fileId, AccessType accessType, 
        ...
        // Verify ownership
        if (!verifyFileOwnership(ownerId, fileId)) {
            AuditLogger.logSecurityEvent("UNAUTHORIZED_LINK_CREATION", 
                                      String.format("User %d attempted to create link for file %d", ownerId, fileId), 
                                      null);
            return null;
        }
        
        // Generate unique access token
        String accessToken = HealthIdGenerator.generateAccessToken();
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsInserted > 0) {
                AuditLogger.logFileSharing(ownerId, fileId, "Temporary link", "SHARED");
                return generateShareUrl(accessToken);
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Temporary link creation failed: " + e.getMessage());
        }
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    // Log access
                    AuditLogger.logFileAccess(0, info.getFileId(), "ACCESSED_VIA_TOKEN", null);
                    
                    return Optional.of(info);
                }
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Access token validation failed: " + e.getMessage());
        }
    ...
    public List<SharedFileInfo> getFilesSharedWithUser(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Get shared files failed: " + e.getMessage());
        }
    ...
    public List<SharedFileInfo> getFilesSharedByUser(int userId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Get shared by user failed: " + e.getMessage());
        }
    ...
    public boolean revokeSharing(int ownerId, int fileId, int sharedWithUserId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logFileSharing(ownerId, fileId, "User ID: " + sharedWithUserId, "REVOKED");
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Revoke sharing failed: " + e.getMessage());
        }
    ...
    public boolean revokeAccessToken(int ownerId, String accessToken) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logFileSharing(ownerId, 0, "Token: " + accessToken, "REVOKED");
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Revoke token failed: " + e.getMessage());
        }
    ...
    public boolean extendSharing(int ownerId, int fileId, int sharedWithUserId, LocalDateTime newExpiresAt) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logFileSharing(ownerId, fileId, "User ID: " + sharedWithUserId, "EXTENDED");
                return true;
            }
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Extend sharing failed: " + e.getMessage());
        }
    ...
    public int cleanupExpiredSharing() {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            if (rowsUpdated > 0) {
                AuditLogger.logSystemMaintenance("CLEANUP_EXPIRED_SHARING", 
                                              String.format("Cleaned up %d expired sharing links", rowsUpdated));
            }
            
            return rowsUpdated;
        } catch (SQLException e) {
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Cleanup expired sharing failed: " + e.getMessage());
        }
    ...
    private boolean verifyFileOwnership(int userId, int fileId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "File ownership check failed: " + e.getMessage());
        }
    ...
    private boolean sharingExists(int fileId, int sharedWithUserId) {
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Sharing existence check failed: " + e.getMessage());
        }
    ...
    private boolean updateExistingSharing(int fileId, int sharedWithUserId, 
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            AuditLogger.logSystemEvent("DATABASE_ERROR", "Update existing sharing failed: " + e.getMessage());
        }
