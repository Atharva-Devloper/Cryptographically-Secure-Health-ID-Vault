...
public class DashboardView {
    ...
    private final FileVaultService fileVaultService = new FileVaultService();
    ...
    // My Files tab
    private final TableView<MedicalFile> filesTable      = new TableView<>();
    private final TableView<MedicalFile> recentTable     = new TableView<>();
    ...
    private VBox buildUploadTab() {
        ...
        int r = 0;
        form.addRow(r++, fLabel("Selected Files:"),    selectedFilesLabel, browseBtn);
        form.addRow(r++, fLabel("Category:"),           uploadCategoryBox);
        form.addRow(r++, fLabel("Doctor Name:"),        doctorNameField);
        form.addRow(r++, fLabel("Hospital:"),           hospitalNameField);
        form.addRow(r++, fLabel("Date:"),               uploadDatePicker);
        form.addRow(r++, fLabel("Description:"),        descriptionArea);
        form.addRow(r++, fLabel("Tags (comma-sep):"),   tagsField);
        form.addRow(r,   new Label(),                   uploadProgressBar, uploadStatusLabel);

        HBox actions = new HBox(12, uploadBtn, clearBtn);
        actions.setPadding(new Insets(0, 0, 0, 20));

        VBox container = new VBox(12, sectionLabel("Upload Medical File"), form, actions);
    ...
    private VBox buildProfileTab() {
        ...
        qrBtn.setOnAction(e -> {
            ...
            AuditLogger.logUserAction(currentUser.getId(), "QR_CODE_GENERATED",
    ...
    private TableView<MedicalFile> buildFilesTableView(boolean withDelete) {
        TableView<MedicalFile> tv = new TableView<>();
    ...
    @SuppressWarnings("unchecked")
    private void buildFilesTableColumns(TableView<MedicalFile> tv, boolean withDelete) {
        tv.getColumns().clear();

        TableColumn<MedicalFile, String> nameCol = new TableColumn<>("File Name");
        nameCol.setCellValueFactory(new PropertyValueFactory<>("originalFileName"));

        TableColumn<MedicalFile, String> catCol = new TableColumn<>("Category");
        catCol.setCellValueFactory(new PropertyValueFactory<>("category"));

        TableColumn<MedicalFile, String> docCol = new TableColumn<>("Doctor");
        docCol.setCellValueFactory(new PropertyValueFactory<>("doctorName"));

        TableColumn<MedicalFile, LocalDate> dateCol = new TableColumn<>("Date");
        dateCol.setCellValueFactory(new PropertyValueFactory<>("uploadDate"));

        TableColumn<MedicalFile, String> sizeCol = new TableColumn<>("Size");
        sizeCol.setCellValueFactory(new PropertyValueFactory<>("formattedFileSize"));

        TableColumn<MedicalFile, Void> actCol = new TableColumn<>("Actions");
    ...
    private void loadDashboardData() {
        try {
            FileVaultService.FileStatistics stats =
            ...
            for (SearchService.CategoryCount c : counts) {
            ...
            // Shared files count
            List<SharingService.SharedFileInfo> shared =
    ...
    private void handleLogout() {
        AuditLogger.logUserAction(currentUser.getId(), "USER_LOGOUT",
                "USER", currentUser.getId(), "User logged out");
    ...
    private void handleSearch() {
        ...
        try {
            ...
            List<MedicalFile> results = searchService.searchMedicalFiles(currentUser.getId(), c);
    ...
    private void handleFilterFiles() {
        try {
            ...
            MedicalFile.FileCategory fileCategory = "All".equals(cat) ? null :
                    MedicalFile.FileCategory.valueOf(cat.replace(" ", "_").toUpperCase());
            List<MedicalFile> files = fileVaultService.getUserFiles(
    ...
    private void handleDownloadFile(MedicalFile file) {
        showInfo("Download", "Would decrypt and download: " + file.getOriginalFileName());
        AuditLogger.logFileAccess(currentUser.getId(), file.getId(), "DOWNLOADED", null);
    }

    private void handleShareFile(MedicalFile file) {
        showInfo("Share", "Would open share dialog for: " + file.getOriginalFileName());
    }

    private void handleDeleteFile(MedicalFile file) {
